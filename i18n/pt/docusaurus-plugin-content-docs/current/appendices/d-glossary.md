...
| **Datasets Raster** | Contentor para ficheiros de imagem Cloud-Optimized GeoTIFF (COG). Apresentado como camadas de sobreposição raster no mapa. |
| **Datasets de Referência** | Contentor para documentos de suporte e tabelas (`.pdf` e `.csv`). Listado na secção de Downloads. |
| **PMTiles** | Ficheiro de tiles vectoriais de ficheiro único servido via HTTP. Gerado a partir de ficheiros vectoriais usando Tippecanoe. |
| **COG** | Cloud-Optimized GeoTIFF. Um formato raster optimizado para armazenamento em nuvem a partir de armazenamento de objectos. Deve estar em EPSG:3857 com tamanho de bloco 256×256 para a renderização raster da plataforma. |
| **presentation_order** | Campo inteiro no Modelo de Dados e nos Cenários que controla a ordem de apresentação na barra lateral do frontend e no menu suspenso. Os números menores aparecem primeiro. |
...
| **GDAL / ogr2ogr** | Biblioteca de dados geoespaciais para conversão de formatos e reprojecção. |
