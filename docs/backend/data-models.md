...
2. Fill in the top-level fields (see the fields table below).
3. Add JSON configuration for `filter_fields`, `popup_fields`, `summary_fields`, and `color_coding` (see [JSON Configuration Fields](./json-configuration)).
...
| **Color Coding (JSON)** | Maps each visualization column value to a hex colour. See [color_coding](./json-configuration#color_coding). |
| **Filter Fields (JSON)** | Controls filter controls in the left panel. See [filter_fields](./json-configuration#filter_fields). |
| **Popup Fields (JSON)** | Defines attributes shown when a user clicks a map feature. See [popup_fields](./json-configuration#popup_fields). |
| **Summary Fields (JSON)** | Specifies aggregated statistics in the right-panel summary. See [summary_fields](./json-configuration#summary_fields). |
