...
3. Add JSON configuration for `filter_fields`, `popup_fields`, `summary_fields`, and `color_coding` (see [Section 5](./json-configuration)).
...
| **Color Coding (JSON)** | Maps each visualization column value to a hex colour. See [Section 5.4](./json-configuration#54-color_coding). |
| **Filter Fields (JSON)** | Controls filter controls in the left panel. See [Section 5.1](./json-configuration#51-filter_fields). |
| **Popup Fields (JSON)** | Defines attributes shown when a user clicks a map feature. See [Section 5.2](./json-configuration#52-popup_fields). |
| **Summary Fields (JSON)** | Specifies aggregated statistics in the right-panel summary. See [Section 5.3](./json-configuration#53-summary_fields). |
